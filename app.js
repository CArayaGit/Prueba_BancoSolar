...
const fs = require('fs');
const { crearUsuario, consultaUsuarios, editUsuario, eliminarUsuario, transferencia, consultaTransferencia } = require('./db/pool.js');
...
const server = http.createServer(async (req, res) => {
    ...
    if(req.url.includes("/usuario") && req.method === "PUT"){
        ...
        req.on("end", async() => {
            //const datos = Object.values(JSON.parse(body));
            //const result = await editUsuario(datos);
            ...
            if(result.data.length === 0){
                ...
                return res.end(JSON.stringify({
                    error: "ID no encontrado en BBDD",
                }))
            }
            res.writeHead(200, {"Content-type": "application/json"});
            res.end(JSON.stringify(result.data));
